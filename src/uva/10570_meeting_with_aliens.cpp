...
std::size_t min_n_swaps(const It first, const It last, const bool reversed)
{
	...
	{
		if (!reversed)
			std::rotate_copy(first, first + i, last, range.begin());
		else
			std::rotate_copy(first, first + i, last, range.rbegin());
		...
		if (n_swaps == 0)
			return 0;
		min_n_swaps = std::min(min_n_swaps, n_swaps);
	}
...
private:
	...
	{
		return read_size_vec(
			positions_, [](std::size_t i) { return i - 1; }); // to zero-based indexing
	}
