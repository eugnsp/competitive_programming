...
std::istream& operator>>(std::istream& in, Road& road)
{
	in >> road.loc1 >> road.loc2 >> road.cost;
...
std::pair<Cost, Size> total_cost_and_n_airports(Size n_locations, 
	Cost airport_cost, const std::vector<Road>& roads)
{
...
private:
	...
	virtual void solve(std::ostream& out, unsigned int i_case) const override
	{
		...
		out << "Case #" << i_case + 1 << ": " << a.first << ' ' << a.second << '\n';
	}
