...
{
public:
	Paths(const Adjacency_list& graph) : graph_(graph)
	{
...
private:
	void depth_first_search(Size vertex, Size parent, Size depth = 0)
	{
		...
		for (const auto& succ : graph_[vertex])
			if (succ.first != parent)
			{
				lengths_[succ.first] = lengths_[vertex] + succ.second;
				depth_first_search(succ.first, vertex, depth + 1);
				add_to_lists(vertex, depth);
			}
	}
...
private:
	virtual bool read_input(std::istream& in) override
	{
		...
		assert(1 <= n_queries && n_queries <= 100'000);
		read_vector(in, n_queries, quieries_);
	...
	{
		const Paths paths(tunnels_);

		const auto path_length =
			[&paths](const Path& path) { return paths.length(path); };

		print_join_transform(out, quieries_.begin(), quieries_.end(), path_length);
		out << '\n';
	}
